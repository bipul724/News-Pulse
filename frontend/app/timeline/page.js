          ...
          <div className="flex flex-col gap-4 rounded-xl border border-stone-200 bg-white p-4 shadow-sm">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
              <div className="relative flex-1">
                <svg className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-stone-400" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
                ...
                <input
                  ...
                  className="h-10 w-full rounded-lg border border-stone-200 bg-paper pl-9 pr-24 text-sm outline-none transition-colors placeholder:text-stone-400 focus:border-accent-400 focus:bg-white focus:ring-4 focus:ring-accent-100"
                />
                <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-xs text-stone-400">
                  {matchingIds ? `${matchingIds.size} match${matchingIds.size === 1 ? '' : 'es'}` : <kbd className="rounded border border-stone-200 bg-white px-1.5 font-sans">/</kbd>}
                </span>
              </div>

              <div className="flex flex-wrap items-center gap-2">
                <Segmented label="Time range" options={RANGES} value={range} onChange={setRange} />
                <label className="flex h-10 cursor-pointer select-none items-center gap-2 rounded-lg border border-stone-200 px-3 text-sm text-stone-600 hover:bg-paper">
                  <input type="checkbox" checked={hideSingles} onChange={e => setHideSingles(e.target.checked)} className="h-4 w-4 accent-accent-600" />
                  2+ articles only
                </label>
                <Segmented
                  label="View"
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-stone-100 pt-4">
              ...
              />
              {filtersActive && (
                <button onClick={resetFilters} className="text-xs font-semibold text-accent-600 hover:text-accent-800">
                  Reset filters
                </button>
              )}
            </div>
          ...
          {loading ? (
            <EmptyCard>
              <div className="w-full max-w-2xl animate-pulse space-y-4">
                ...
                <div className="ml-auto h-8 w-3/4 rounded bg-stone-100" />
                <div className="h-8 w-1/2 rounded bg-stone-100" />
                <div className="ml-12 h-6 w-2/3 rounded bg-stone-100" />
              </div>
            </EmptyCard>
          ) : timelineData.length === 0 ? (
            <EmptyCard>
              <h3 className="mb-2 text-lg font-semibold text-stone-700">No news activity yet</h3>
              <p className="mb-6 max-w-sm text-sm text-stone-500">Run Refresh Data to fetch the latest stories and build the timeline.</p>
              <RefreshButton onRefreshComplete={fetchTimeline} apiUrl={API_URL} />
            </EmptyCard>
          ) : filteredTimeline.length === 0 || (view === 'list' && listItems.length === 0) ? (
            <EmptyCard>
              <h3 className="mb-2 text-base font-semibold text-stone-700">Nothing matches these filters</h3>
              <p className="mb-5 text-sm text-stone-500">Try a wider time range, another source, or a different search.</p>
              <button onClick={resetFilters} className="rounded-lg bg-stone-900 px-4 py-2 text-sm font-semibold text-white hover:bg-stone-800">
                Reset filters
              </button>
            </EmptyCard>
          ) : view === 'timeline' ? (
            ...
            <>
              <div className="mb-3 flex items-center justify-between">
                <p className="text-sm text-stone-500">
                  <span className="font-semibold text-stone-700">{listItems.length}</span> topics
                </p>
...
function Segmented({ label, options, value, onChange, compact }) {
  return (
    <div role="group" aria-label={label} className={`flex items-center rounded-lg border border-stone-200 bg-paper p-0.5 ${compact ? 'h-8' : 'h-10'}`}>
      {options.map(opt => (
        <button
          key={opt.key}
          ...
          className={`h-full rounded-md px-3 text-sm font-medium transition-colors ${
            value === opt.key ? 'bg-white text-stone-900 shadow-sm' : 'text-stone-500 hover:text-stone-800'
          }`}
        >
          {opt.label}
        </button>
...
function EmptyCard({ children }) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center rounded-xl border border-stone-200 bg-white p-12 text-center shadow-sm">
      {children}
    </div>
  );
}
